resource "aws_autoscaling_group" "brz_asg" {
  name_prefix           = "brz-auto-scaling-group-"
  min_size              = 1
  max_size              = 3
  desired_capacity      = 2
  vpc_zone_identifier   = [
  ...
  launch_template {
    id      = aws_launch_template.brz_LT.id
    version = "$Latest"
  }
...
resource "aws_autoscaling_policy" "app1_scaling_policy" {
  ...
  autoscaling_group_name = aws_autoscaling_group.brz_asg.name
